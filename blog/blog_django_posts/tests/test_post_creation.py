...
from blog_django_posts.models import Post
...
class PostCreationTest(TestCase):
    ...
    def test_post_creation_access(self):
        response = self.client.get(
            reverse('blog_django_posts:post_creation')
        )
        self.assertEqual(302, response.status_code)
        self.assertURLEqual(response.url, reverse('login') + '?next=/post-creation/')

        self.client.login(username=self.user.username, password=self.password)
        response = self.client.get(reverse('blog_django_posts:post_creation'))
    ...
    def test_post_creation(self):
        ...
        self.assertURLEqual(
            response.url,
            reverse('blog_django_posts:post_details', kwargs={"pk": post.pk})
        )